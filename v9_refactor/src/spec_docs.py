...
METHOD_UID_NAMES = {
    # Session Manager layer methods (Table 241, core/5.2.1)
    "000000000000FF01": "Properties",
    "000000000000FF02": "StartSession",
    "000000000000FF03": "SyncSession",
    "000000000000FF05": "StartTrustedSession",
    "000000000000FF06": "SyncTrustedSession",
    # Base Template methods (opal/4.2.1.4, opal/4.3.1.5)
...
COLUMN_NAME_NUMBERS.update({
    ...
    "Template": {"uid": 0, "name": 1, "revisionnumber": 2, "revision": 2, "instances": 3, "maxinstances": 4},
    ...
    "ClockTime": {
        # core/5.5.3.1 Table 218: exact column-to-number mapping
        "uid": 0,
        "havehigh": 1,
        "highbywhom": 2,
        "highsettime": 3,
...
READ_ONLY_COLUMNS = {
    "C_PIN": {0, 1, 2},
    # Authority: IsClass (3) is non-modifiable per core/5.3.2.10.4.
    # CommonName (2) IS writable via ACE_Admins_Set_CommonName / ACE_UserN_Set_CommonName (opal/4.3.1.7).
    "Authority": {0, 1, 3},
    "ACE": {0, 1, 2},
    "AccessControl": set(COLUMN_NAME_NUMBERS["AccessControl"].values()),
    # Locking: ReEncryptState (12), LastReEncryptLBA (17), LastReEncState (18), GeneralStatus (19)
    # are drive-maintained read-only status columns.
    # CommonName (2) IS writable by Admins via ACE_Admins_Set_CommonName (opal/4.3.1.7 Table 39).
    "Locking": {0, 1, 12, 17, 18, 19},
    "LockingInfo": set(COLUMN_NAME_NUMBERS["LockingInfo"].values()),
    "MBRControl": {0},
    "MediaKey": {0, 1, 2},
    # core/5.4.2.4.1-4.7: SP identity/lifecycle columns are host-immutable.
    # Frozen (7) is deliberately NOT in 5.4.2.4's immutability list — freezing
    # an SP is done by setting it (core/4.5.3); only the Admin SP is protected
    # from Disabled/Frozen by access control (core/5.4.5.1).
    "SP": {0, 1, 2, 3, 4, 5, 6},
    # SPInfo identity and capacity counters are TPer-maintained.  Enabled (6)
    # is explicitly host-modifiable by the SP Owner (core/5.3.2.1.7), so do
    # not mark the whole SPInfo table read-only; only prevent host-forged
    # identity/Size/SizeInUse values from poisoning capacity evidence.
    "SPInfo": {0, 1, 2, 3, 4},
    # core/5.3.2.2.1-2.4: all SPTemplates columns are host-immutable.
    "SPTemplates": {0, 1, 2, 3},
    # core/5.4.2.3.1-3.12: all Template columns are host-immutable.
    "Template": {0, 1, 2, 3, 4},
...
RULE_REFERENCES = {
    ...
    "authenticate": ("core/5.3.3.12", "core/5.3.4.1.2.1", "core/5.3.4.1.14", "core/5.3.4.1.14.1"),
    ...
    "row_management": ("core/5.3.3.4", "core/5.3.3.4.1", "core/5.3.3.4.3", "core/5.3.3.5", "core/5.3.3.5.1", "core/5.3.3.5.2", "core/5.3.3.5.2.1", "core/5.3.3.5.3", "core/5.3.4.2.3", "core/5.3.4.2.4"),
    ...
    "method_table": ("opal/4.2.1.4", "opal/4.3.1.5"),
    ...
    "issue_sp": ("core/5.4.3.1", "core/5.4.3.1.1", "core/5.4.3.1.2", "core/5.4.3.1.3", "core/5.4.3.1.5", "core/5.4.3.1.6", "core/5.4.3.1.7"),
...
def access_control_refs_from_value(value):
    ...
    for item in raw_items:
        ...
        if not item_text:
            continue
        is_uid_text = bool(re.match(r"^[0-9A-Fa-f\s\-]+$", item_text))
        compact = compact_uid(item_text) if is_uid_text else None
            ...
            for row in block:
                ...
                if family == "ACE":
                    ...
                    policy["access_control_rows"].append({
                        ...
                        "ace_refs": access_control_refs_from_value(ace_value),
                        "add_ace_acl_refs": access_control_refs_from_value(row_lookup(row, "AddACEACL")),
                        "remove_ace_acl_refs": access_control_refs_from_value(row_lookup(row, "RemoveACEACL")),
                        "get_acl_acl_refs": access_control_refs_from_value(row_lookup(row, "GetACLACL")),
                        "delete_method_acl_refs": access_control_refs_from_value(row_lookup(row, "DeleteMethodACL")),
                        "source": source,
                    })
...
def _wildcard_access_control_rows():
    """AccessControl rows for wildcard UID patterns absent from spec_index.

    The spec defines (+NN NN) and (TT TT TT TT) patterns for objects that come
    in families larger than one row — additional locking ranges, additional
    media keys, and arbitrary authority objects.  The spec_index only stores
    explicit entries for the first concrete instance of each (Range1, Admin1,
    User1).  These wildcard rows cover the remainder so that the ACE policy
    engine fires instead of falling through to hardcoded family checks.

    Each row uses a ``uid_prefix`` field (checked by access_row_matches) to
    match any object whose compact UID begins with that hex prefix.
    """
    lsp = "opal/4.3.1.6"   # LockingSP source scope
    asp = "opal/4.2.1.5"   # AdminSP source scope
    return [
        # ── Locking RangeNNNN Set (Range2+) ─────────────────────────────────
        # InvokingID "00 00 08 02 00 03 00 00 (+NN NN)": covers Range2 and above.
        # Range1 (0000080200030001) and GlobalRange (0000080200000001) are covered
        # by explicit invoking_uid rows and matched first.
        {
            "uid_prefix": "000008020003",
            "method": "Set",
            "ace_refs": [
                "acelockingadminsrangestarttolor",
                "acelockingrangennnnsetrdlocked",
                "acelockingrangennnnsetwrlocked",
                "aceadminssetcommonname",
            ],
            "source": lsp,
        },
        # ── K_AES_128 RangeNNNN Get / GenKey (Range2+) ──────────────────────
        {
            "uid_prefix": "000008050003",
            "method": "Get",
            "ace_refs": ["acekaesmode"],
            "source": lsp,
        },
        {
            "uid_prefix": "000008050003",
            "method": "GenKey",
            "ace_refs": ["acekaes128rangennnngenkey"],
            "source": lsp,
        },
        # ── K_AES_256 RangeNNNN Get / GenKey (Range2+) ──────────────────────
        {
            "uid_prefix": "000008060003",
            "method": "Get",
            "ace_refs": ["acekaesmode"],
            "source": lsp,
        },
        {
            "uid_prefix": "000008060003",
            "method": "GenKey",
            "ace_refs": ["acekaes256rangennnngenkey"],
            "source": lsp,
        },
        # ── Authority wildcard Get ───────────────────────────────────────────
        # LockingSP: InvokingID "00 00 00 09 TT TT TT TT" → ACE_Authority_Get_All
        # + ACE_Anybody_Get_CommonName.  Explicit Admin1 and User1 rows still match
        # first via their invoking_uid; this catches every other authority UID.
        {
            "uid_prefix": "00000009",
            "method": "Get",
            "ace_refs": ["aceauthoritygetall", "aceanybodygetcommonname"],
            "source": lsp,
        },
        # AdminSP: InvokingID "00 00 00 09 TT TT TT TT" → ACE_Anybody.
        # Any open AdminSP session may read any authority object (opal/4.2.1.5).
        {
            "uid_prefix": "00000009",
            "method": "Get",
            "ace_refs": ["aceanybody"],
...
def _augment_access_policy(policy):
    augmented = dict(policy)
    augmented["access_control_rows"] = (
        list(policy.get("access_control_rows") or []) + _wildcard_access_control_rows()
    )
    return augmented
...
def default_access_policy():
    ...
    if not policy:
        policy = build_access_policy_from_index(index if isinstance(index, dict) else {})
    return _augment_access_policy(policy)
