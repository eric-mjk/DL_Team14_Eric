        ...
        for block in table.get("blocks", []):
            ...
            for row in block:
                ...
                if family == "ACE":
                    ...
                    refs = []
                    # Flatten: spec tables sometimes store multiple ACE names as a
                    # comma-separated string rather than a list.
                    if isinstance(ace_value, list):
                        raw_refs = ace_value
                    elif isinstance(ace_value, str) and "," in ace_value:
                        raw_refs = [s.strip() for s in ace_value.split(",")]
                    else:
                        raw_refs = [ace_value]
                    for item in raw_refs:
                        item_text = str(item or "").strip()
                        # Only treat as a hex UID when the original text contains nothing but
                        # hex digits and separators (spaces / dashes).  ACE names like
                        # "ACE_Locking_GlobalRange_Get_RangeStartToActiveKey" contain letters
                        # outside a-f which betray them as names, not UIDs.
                        import re as _re
                        is_uid_text = bool(_re.match(r'^[0-9A-Fa-f\s\-]+$', item_text)) if item_text else False
