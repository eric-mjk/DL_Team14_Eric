...
**Bug 1 — Revert ACL: SID only → SID OR Admin**
- `_expected_revert`가 SID authority만 허용했음
- spec opal/5.1.2: ACE_SP_SID OR ACE_Admin — SID 또는 Admin 중 하나면 충분
- `_has_authority(SID) or _has_admin_authority()` 로 수정
- 관련 합성 케이스: `syn_pass_31_activate_rejected`, `syn_fail_31_activate_no_sid`

**Bug 2 — C_PIN PIN 컬럼 읽기: MSID만 예외**
- `_can_read_cpin_col3`가 SID/Admin1도 자기 C_PIN PIN을 읽을 수 있다고 판단했음
- spec opal/4.2.1.5:
  - `ACE_C_PIN_SID_Get_NOPIN` — 이름에 NOPIN, SID 자신도 불가
  - `ACE_C_PIN_Admins_Get_All_NOPIN` — LockingSP Admin도 불가
  - `ACE_C_PIN_MSID_Get_PIN` — MSID만 Anybody가 읽을 수 있음
- MSID 외 모든 C_PIN PIN 컬럼은 읽기 불가로 수정
- 관련 합성 케이스: `syn_pass_25`, `syn_pass_26`, `syn_pass_27`, `syn_pass_27b`, `syn_fail_25`, `syn_fail_25b`, `syn_fail_26`, `syn_fail_27`

**Bug 3 — GET Locking: 인증 불필요 → Admins 인증 필요**
- `POLICIES[("GET", "LOCKING")]`에 `require_authenticated=False`였음
- spec opal ACE_Locking_RangeN_Get_RangeStartToActiveKey = Admins only
- `require_authenticated=True`, `allowed_authorities=_LOCKING_ADMINS` 로 수정
- 관련 합성 케이스: `syn_pass_17_locking_get_unauth`, `syn_fail_17_locking_get_unauth`

**Bug 4 — SET Locking: Admins 인증 필요 (확인)**
- spec opal ACE_Locking_RangeN_Set_* = Admins only
- `POLICIES[("SET", "LOCKING")]` 동일하게 Admins 필요로 확인 및 유지

검증: public 20/20 (100%) 유지, synthetic 57/57 (100%) 유지.

---

## v3 Error Notes (역사적 기록 — 구 아키텍처)
