...
class ProtocolState:
    ...
    deleted_credentials: set[str] = field(default_factory=set)  # UIDs explicitly invalidated (e.g. GenKey on C_PIN)
...
class ExpectedOutcome:
    ...
    read_result_must_differ_from: str | None = None  # GenKey: actual must not equal this old value
...
class Solver:
    ...
    def _ingest_activate(self, step: dict[str, Any]) -> None:
        ...
        if object_uid:
            ...
            if object_uid == LOCKING_SP:
                ...
                # LockingSP authorities Admin2-4, User1-8 start with empty PINs.
                # Pre-populate so that StartSession with no HostChallenge is recognised as empty-PIN auth.
                _LOCKING_EMPTY_CRED_UIDS = [
                    "0000000900010002", "0000000900010003", "0000000900010004",  # Admin2-4
                    "0000000900030001", "0000000900030002", "0000000900030003",  # User1-3
                    "0000000900030004", "0000000900030005", "0000000900030006",  # User4-6
                    "0000000900030007", "0000000900030008",                      # User7-8
                ]
                for _auth_uid in _LOCKING_EMPTY_CRED_UIDS:
                    self.state.credentials.setdefault(_auth_uid, "")
        ...
        if status == "SUCCESS":
            ...
            if self._object_name(step) == "C_PIN":
                ...
                if object_uid:
                    ...
                    self.state.deleted_credentials.add(object_uid)
                    # Credential is aliased under both C_PIN UID and authority UID; clear both.
                    authority_uid = self._authority_for_credential_uid(object_uid)
                    if authority_uid:
                        self.state.credentials.pop(authority_uid, None)
                        self.state.deleted_credentials.add(authority_uid)
            else:
    ...
    def _expected_start_session(self, step: dict[str, Any]) -> ExpectedOutcome:
        ...
        # LockingSP must be activated before sessions can be opened.
        if spid == LOCKING_SP and LOCKING_SP not in self.state.activated_sps:
        ...
        # Empty-PIN authority: any absent/empty challenge succeeds.
        if credential == "" and (challenge is None or self._values_equal(challenge, "")):
            return ExpectedOutcome(status="SUCCESS")

        # Known credential, matching challenge → SUCCESS.
        if credential is not None and challenge is not None and self._values_equal(challenge, credential):
            return ExpectedOutcome(status="SUCCESS")

        # Credential explicitly deleted (e.g. GenKey on C_PIN) → always NOT_AUTHORIZED.
        credential_uid = self._credential_uid_for_authority(authority)
        if credential is None and (
            authority in self.state.deleted_credentials
            or (credential_uid is not None and credential_uid in self.state.deleted_credentials)
        ):
            return ExpectedOutcome(status="NOT_AUTHORIZED")

        # Credential unknown (never tracked) and no challenge → ambiguous; SSD may accept or reject.
        if credential is None and challenge is None:
            return ExpectedOutcome(allowed_statuses={"SUCCESS", "NOT_AUTHORIZED"})

        # Credential unknown and challenge present → SSD decides.
    ...
    def _expected_activate(self, step: dict[str, Any]) -> ExpectedOutcome:
        # Activate LockingSP requires: AdminSP session, SID authority, write mode.
        # NOTE: The POLICIES lookup uses object name "LOCKINGSP", but real invoking_id.name
        # is "SP" — so we check auth requirements here directly instead.
        target_uid = normalize_uid(get_path(step, "input", "invoking_id", "uid"))
        ...
        # Fallback: policy-based check for non-standard target UIDs.
    ...
    def _expected_revert(self, step: dict[str, Any]) -> ExpectedOutcome:
        ...
        if self.state.session.spid != ADMIN_SP:
            return ExpectedOutcome(status="NOT_AUTHORIZED")
        # Spec ACL: ACE_SP_SID OR ACE_Admin — either SID or an Admin authority suffices
        if not self._has_authority(SID_AUTHORITY) and not self._has_admin_authority():
            return ExpectedOutcome(status="NOT_AUTHORIZED")
        target_uid = normalize_uid(get_path(step, "input", "invoking_id", "uid"))
    ...
    def _expected_read(self, step: dict[str, Any]) -> ExpectedOutcome:
        ...
        if self._read_locked(step):
            return ExpectedOutcome(interface_result_should_fail=True)
        if self._media_key_changed_for_read(step):
            # Key changed — old plaintext is unreadable.
            if self.state.pre_genkey_read_result is not None:
                # Had a prior Read → the new read must return different data.
                return ExpectedOutcome(status=None, read_result_differs_previous=True)
            # No prior Read, but may have a prior Write at this LBA.
            lba_key = self._lba_key(step)
            old_written = self.state.user_data.get(lba_key)
            if old_written is not None:
                # Old plaintext was written; after GenKey it must not be readable as-is.
                return ExpectedOutcome(status=None, read_result_must_differ_from=old_written)
            return ExpectedOutcome(status=None)
    ...
    def _compare_with_actual(self, step: dict[str, Any], expected: ExpectedOutcome) -> bool:
        ...
        if self._method_name(step) == "READ":
            ...
            if expected.read_result_equals_previous:
                return normalized == self.state.pre_genkey_read_result
            if expected.read_result_must_differ_from is not None:
                return normalized != expected.read_result_must_differ_from
        ...
        if self._method_name(step) in {"GET", "PROPERTIES"}:
            ...
            for key, expected_value in expected.required_values.items():
                ...
                if expected_value is not None:
                    # Coerce both to bool for boolean comparisons, else compare directly.
                    actual_val = values[key]
                    if isinstance(expected_value, (bool, int)) and to_bool(actual_val) is not None:
                        if to_bool(actual_val) != bool(expected_value):
                            return False
    ...
    # Boolean locking columns whose values we verify against tracked state.
    # Numeric/string columns (RangeStart, RangeLength) use hex strings in responses
    # and integer in state — skip value validation there to avoid type mismatches.
    _LOCKING_BOOL_COLS = frozenset(["5", "6", "7", "8", "9"])
    ...
    def _expected_locking_get(self, step: dict[str, Any]) -> ExpectedOutcome:
        ...
        expected_columns = self._expected_columns_for_range(step, semantic.readable_column_sets)
        # If we have tracked boolean state for this range, validate returned values match.
        object_uid = normalize_uid(get_path(step, "input", "invoking_id", "uid"))
        tracked = self.state.object_values.get(object_uid, {}) if object_uid else {}
        for key in expected_columns:
            tracked_val = tracked.get(key)
            if tracked_val is not None and key in self._LOCKING_BOOL_COLS:
                expected.required_values[key] = tracked_val
            else:
                expected.required_values[key] = None  # just check key is present
        return expected
    ...
    def _can_read_cpin_col3(self, object_uid: str | None) -> bool:
        """Return True if current session/authority may read column 3 (PIN) of the given C_PIN object.

        Per Opal spec AccessControl tables:
        - AdminSP C_PIN_MSID: ACE_C_PIN_MSID_Get_PIN → Anybody (PIN readable by all in AdminSP)
        - AdminSP C_PIN_SID: ACE_C_PIN_SID_Get_NOPIN → Admins OR SID, but Columns excludes PIN
        - LockingSP all C_PINs: ACE_C_PIN_Admins_Get_All_NOPIN → Admins only, Columns excludes PIN

        Result: ONLY C_PIN_MSID's PIN column is readable. All other C_PIN PINs are NOT readable.
        """
        if object_uid is None:
            return False
        # MSID: readable by Anybody (unauthenticated) in AdminSP.
        if object_uid == C_PIN_MSID:
            return self.state.session.spid == ADMIN_SP
        # All other C_PIN PIN columns are not readable per spec (no ACE covers them for GET).
        return False
    ...
    def _credential_for_authority(self, authority: str | None) -> str | None:
        ...
        credential_uid = self._credential_uid_for_authority(authority)
        if authority == SID_AUTHORITY:
            # Use explicit None checks so empty string "" (no-password) is returned correctly.
            for key in (SID_AUTHORITY, C_PIN_SID, C_PIN_MSID):
                val = self.state.credentials.get(key)
                if val is not None:
                    return val
            return None
        if credential_uid is None:
            return None
        # Prefer the authority-keyed entry; fall back to C_PIN UID entry.
        # Must use explicit None check: empty string "" is a valid credential.
        val = self.state.credentials.get(authority)
        if val is not None:
            return val
        return self.state.credentials.get(credential_uid)
    ...
    def _lba_key_in_erased_range(self, lba_key: str, range_uid: str) -> bool:
        """Return True if the LBA range described by lba_key overlaps with range_uid."""
        if range_uid == LOCKING_GLOBAL_RANGE:
            return True
        bounds = self._lba_bounds_from_key(lba_key)
        if bounds is None:
            return True  # conservative: assume overlap
        values = self.state.object_values.get(range_uid, {})
        return self._range_overlaps(bounds, range_uid, values)

    def _lba_bounds_from_key(self, lba_key: str) -> tuple[int, int] | None:
        parts = lba_key.replace("~", "-").replace("..", "-").split("-")
        if len(parts) == 1:
            start = to_int(parts[0].strip())
        ...
        return self._freeze(
            {
                ...
                "deleted_credentials": self.state.deleted_credentials,
