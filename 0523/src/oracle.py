...
from .spec_docs import COLUMN_LIMITS, METHOD_NAMES, max_column_for_family, read_only_columns_for_family, refs_for
...
def authority_classes_for(state, authority):
    classes = set()
    for record in authority_records_for(state, authority):
    ...
    if isinstance(authority, str) and authority.startswith("Admin") and authority_enabled(state, "Admins"):
        classes.add("Admins")
    if isinstance(authority, str) and authority.startswith("User") and authority_enabled(state, "Users"):
        classes.add("Users")
    return classes
