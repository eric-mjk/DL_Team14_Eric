...
class LLMParseFallback:
    ...
    def _build_prompt(
        ...
        return (
            "Analyze this SSD protocol step. If the deterministic parser missed a protocol "
            "object/method, infer the canonical event and any state mutation caused by a "
            "successful pre-target step. For target judging, decide whether the observed "
            ...
            "\"confidence\": 0.0-1.0, "
            "\"reason\": \"short\", "
            "\"normalized_event\": object|null, "
            "\"state_patch\": object|null, "
            "\"verdict\": \"pass\"|\"fail\"|null"
            "}\n\n"
            "Allowed event patch fields are the normalized event fields used by the v8 state "
            "machine, including method/object/session/auth/cellblock/status fields.\n"
            "Allowed state_patch keys: session, credentials, sp_lifecycle, locking_sp_active, "
            "locking_ranges, mbr, trylimit_by_authority, failed_auth_counts, writes.\n\n"
...
_SYSTEM_PROMPT = """\
You are an expert parser for TCG Storage Core and Opal SSC traces.
The deterministic parser may fail on new hidden protocol objects.
Infer only conservative canonical fields and state mutations supported by the raw step.
Never invent unrelated state. Failed commands normally do not mutate state, but they may
provide evidence about credentials, lockout, authorization, object identity, or whether a
final response is protocol-compliant. Use failed_observations in the state snapshot.
...
def _extract_prompt_rag_context(prompt: str) -> str | None:
    marker = '"retrieved_spec_context": '
    idx = prompt.find(marker)
    if idx < 0:
        return None
    max_chars = int(os.environ.get("LLM_PARSE_AUDIT_RAG_CHARS", "12000"))
    return _truncate(prompt[idx : idx + max_chars], max_chars)
