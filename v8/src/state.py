...
from .normalizer import canonical_sp, compact_uid, find_named_value, is_success_status, to_bool, to_int
...
def merge_table_columns(state, event, columns):
    ...
    if event.get("object_family") == "SPInfo" and 5 in columns:
...
def apply_successful_issue_sp(state, event):
    ...
    allocated_size = to_int(find_named_value(return_values, {"Size"}))
    if allocated_size is None:
        allocated_size = to_int(find_named_value(parameters, {"Size"}))
    if free is not None and allocated_size is not None:
        issuance_space["free"] = max(0, free - allocated_size)
    ...
    sp_uid = (
        compact_uid(find_named_value(return_values, {"UID", "SPID", "SPUID"}))
        or compact_uid(find_named_value(parameters, {"UID", "SPID", "SPUID"}))
    )
...
def apply_event(state, event):
    ...
    if event["kind"] == "discovery":
        if 0x0203 in (event.get("features") or {}):
