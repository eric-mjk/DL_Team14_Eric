...
The normal submission default (`state_machine.env`) does **not** use these prompts because it disables `USE_LLM_PARSE_FALLBACK`.

## Main LLM parse fallback

Used by `src/llm_parse_fallback.py` when `SOLVER_PROFILE=submission` or another profile sets `USE_LLM_PARSE_FALLBACK=1`.
...
Used by `src/rag_parser_repair.py` when legacy `ENABLE_RAG_REPAIR=1` is selected.
